...
#             filter(Exposure > 0.2)
# saveRDS(aeBase, "Figure3.RData")

aeBase <- readRDS("Figure3.RData") %>%
            filter(Exposure > 0.2)
```
...
plot <- createQtlPlot(
        ...
        labs(
          title="Figure 3: QTL plot for AE rates in study WA29767",
          y="Density"
        ) +
        qtlScale +
...
#Save to file
tiff(filename="Figure3.tiff", width=4500, height=3000, res=800)
print(plot)
dev.off()
